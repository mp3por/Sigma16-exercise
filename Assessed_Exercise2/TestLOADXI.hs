------------------------------------------------------------------------
-- TestLOADXI: A program to test the new 'loadxi' instruction added to
-- the instruction set of the M1 circuit. This instruction does value
-- retrieval and index increment during loops in one go in order to
-- reduce the number of instructions needed to implement this very
-- common pattern.
------------------------------------------------------------------------
module TestLOADXI where
import M1run
...
-- ~~~~~~~~~~~~~~~~  ~~~~ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 [
  ...
  "f504", "0014", -- 0009       jumpf R5,done[R0]   ; if i>=n then goto done
                  ...
                  -- 000b ; if x[i] > max
                  -- 000b
  "f537", "0019", -- 000b       load  R5,x[R3]      ; R5 = x[i] -------------------------> changed to use the new instruction
  ...
  "0450",         -- 0010       add   R4,R5,R0      ; max := x[i]
                  ...
                  -- 0011
  "0000",         -- 0011 next   add   R3,R3,R1     ; i = i + 1 -------------------------> does nothing, but left for easier comparison between debug outputs
